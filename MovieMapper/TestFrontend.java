...
public class TestFrontend {
    ...
    public void runTests() {
        ...
        System.out.print("Test 'r' load ratings selection screen (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
        if (this.testFrontendRForRatings()) {
            System.out.println("PASSED");
        } else {
            System.out.println("FAILED");
        }
        System.out.print("Test frontend lists correct movies with genre selection (WARNING: if 'x' does not exit app, test won't exit and run indefinitely!): ");
        if (this.testFrontendOutputWithGenreMovies()) {
            System.out.println("PASSED");
        } else {
            System.out.println("FAILED");
        }
    ...
    /**
     * This test runs the front end and redirects its output to a string. It then
     * passes in 'g' as a command to go to the genre selection mode. It then exits
    ...
    /**
     * This test runs the front end and redirects its output to a string. It then passes in 'r' as a command to go to
     * the ratings selection mode. It then exits the app by pressing 'x' to go back to the main mode and another 'x'
     * to exit. The test succeeds if the ratings selection screen has all ratings displayed. It fails if
     * the ratings are not all shown, the front end has not been instantiated (is null), or there is an
     * exception.
     * @return true if the test passed, false if it failed
     */
    public boolean testFrontendRForRatings() {
        ...
        try {
            ...
            if (frontend != null && appOutput.contains("0")
                ...
                // test passes if all ratings are displayed on the screen
                return true;
            } else {
                // test failed
                return false;
     ...
     * @return true if the test passed, false if it failed
     */
    public boolean testFrontendOutputWithGenreMovies() {
        PrintStream standardOut = System.out;
        InputStream standardIn = System.in;
        try {
            // set the input stream to our input (with an g to test of the program lists genres)
            String input =
                    "g" + System.lineSeparator() + "1" + System.lineSeparator() + "x" + System.lineSeparator() + "x";
            InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
            System.setIn(inputStreamSimulator);
            ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
            // set the output to the stream captor to read the output of the front end
